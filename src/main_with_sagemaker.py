...
def run_prepare_train_data_task(args, sagemaker_meta):
    ...
    pytorch_processor = PyTorchProcessor(
        ...
        instance_type=args.instance_type,   #컨테이너 생성될 인스턴스 정의
        instance_count=1,
        max_runtime_in_seconds=1 * 60 * 60,
    )

    pytorch_processor.run(  #컨테이너 생성됨
        code="main.py",
        source_dir=".",
...
def run_train_task(args, sagemaker_meta):
    ...
    estimator = PyTorch(
        ...
        disable_profiler=False,   #default=true라서 false로 변경. 모델 학습 성능에 영향을 끼치기 때문에...
        profiler_config=profiler_config,
        rules=rules,
    )

    estimator.fit(   #전처리에서는 run, 학습에서는 fit
        inputs={
            "train": (
                f"{sagemaker_meta.s3_input_src}/"
                f"{args.dataset_name}_train_{args.model_name}.snappy.parquet"
